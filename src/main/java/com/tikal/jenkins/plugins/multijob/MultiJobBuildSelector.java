...
public class MultiJobBuildSelector extends BuildSelector {
	...
	public Run<?, ?> getBuild(Job<?, ?> job, EnvVars env, BuildFilter filter, Run<?, ?> parent) {
		...
		// Are we in the MultiJob itself and trying to get an artifact from a
		// Phase Build?
		if (parent instanceof MultiJobBuild) {
			multiJobBuild = (MultiJobBuild) parent;
		}
		// Nope, look for Upstream MultiJob that triggered this run (Are we in a
		// Phase job?)
		else {
			// Matrix run is triggered by its parent project, so check causes of
			// parent build:
			for (Cause cause : parent instanceof MatrixRun ? ((MatrixRun) parent).getParentBuild().getCauses()
					: parent.getCauses()) {
				UpstreamCause upstreamCause = (UpstreamCause) cause;
				Job upstreamJob = Jenkins.getInstance().getItemByFullName(upstreamCause.getUpstreamProject(),
						Job.class);
		...
		// Get the run for our source Job in the current MultiJob Project's
		// Build
