...
public class MultiJobBuild extends Build<MultiJobProject, MultiJobBuild> {
	...
	public String getBuildParams(SubBuild subBuild) {
		try {
			AbstractProject project = Jenkins.getInstance().getItem(subBuild.getJobName(), this.getParent(),
			...
			for (ParameterValue parameterValue : parameters) {
				...
				buffer.append("<input type='text' size='15' value='").append(name).append("' readonly/>")
						.append("&nbsp;").append("<input type='text' size='35' value='").append(value)
						.append("'/ readonly>").append("</br>");
			}
	...
	public static class SubBuild {
		...
		public String toString() {
			return "SubBuild [parentJobName=" + parentJobName + ", parentBuildNumber=" + parentBuildNumber
					+ ", jobName=" + jobName + ", buildNumber=" + buildNumber + "]";
		}
