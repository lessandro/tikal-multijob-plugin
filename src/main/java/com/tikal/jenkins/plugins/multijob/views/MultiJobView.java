...
public class MultiJobView extends ListView {
	...
	private void addMultiProject(MultiJobProject parent, MultiJobProject project, BuildState buildState, int nestLevel,
	...
	private void addProjectFromBuilder(MultiJobProject project, BuildState buildState, List<TopLevelItem> out,
			BuildStep builder, int phaseNestLevel, boolean isConditional) {
	...
	@SuppressWarnings("rawtypes")
	private void addSimpleProject(MultiJobProject parent, Job project, BuildState buildState, int nestLevel,
			List<TopLevelItem> out) {
		out.add(new ProjectWrapper(parent, project, buildState, nestLevel));
	}

	@SuppressWarnings({ "rawtypes" })
	private BuildState createBuildState(BuildState parentBuildState, MultiJobProject multiJobProject, Job project) {
		...
		return new BuildState(project.getName(), previousBuildNumber, lastBuildNumber, lastSuccessBuildNumber,
	...
	private BuildState createBuildState(MultiJobProject project) {
		...
		return new BuildState(project.getName(), previousBuild == null ? 0 : previousBuild.getNumber(),
	...
	protected void initColumns() {
		try {
			...
			field.set(this, new DescribableList<ListViewColumn, Descriptor<ListViewColumn>>(this,
