...
public final class CounterHelper {
	...
	/**
	 * Put the counters in phase variables, add the counters to the multijob
	 * variables, and merge all in previous variables.
	 *
	 * @param listener
	 *            listener
	 * @param phaseName
	 ...
	 * @return Map
	 */
	public static synchronized Map<String, String> putPhaseAddMultijobAndMergeTheRest(BuildListener listener,
			final String phaseName, final Map<String, String> incomingVars, final Map<String, String> previousEnvVars) {
		...
		for (Map.Entry<String, String> entry : incomingVars.entrySet()) {
			...
			if (counterKey == null) {
				...
				// listener.getLogger().println("putPhase " + phaseName + ": " +
				// counterKey.name() + "/" + counterKey.getPhaseKey() + "/" +
				// counterKey.getMultiJobKey() + " = " + previousValue + "/" +
				// incomingValue);
